...
Each point corresponds to a single week, with deaths reported in thousands. For instance, in the week of 5 May 2018, there were 53,874 deaths nationwide. Our areas of interest are highlighted in the years 2019 (blue) and 2020 (red).
...
The curves are the cumulative weekly all-cause excess mortality (per million) for each U.S. state. Five states receive special focus and are distinguished by color: New Jersey (green), Louisiana (red), California (plum), Maryland (blue) and Texas(salmon). All remaining states are in grey including Puerto Rico and District of Columbia.

From the emphasized states, New Jersey had the highest cumulative excess deaths per million in the US, with most deaths occurring between April and June. 
...
state_covid_tfd <- tfd(cum_covid, arg = num_grid(current_date))
# tfd[52] on (0,357) based on 52 evaluations each
# interpolation by tf_approx_linear 
# [1]: ( 0,0);( 7,0);(14,0); ...
# [2]: ( 0,0);( 7,0);(14,0); ...
...
The curves are the cumulative COVID-19 mortality (per million) for each U.S. state. Five states receive special focus and are distinguished by color: New Jersey (green), Louisiana (red), California (plum), Maryland (blue) and Texas (salmon). All remaining states are in grey including Puerto Rico and District of Columbia.


Notably, while the trajectories across states and territories generally align, COVID-19 deaths tend to be lower than the all-cause excess mortality.
